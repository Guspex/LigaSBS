...
with st.container():
    col_logo, col_title, col_button = st.columns([1, 6, 1])

    with col_logo:
        st.image("logo-figurativa-a-toca.png", width=100)

    with col_title:
        st.title("Plataforma de Troca e Venda de Cartas - Magic: The Gathering")

    with col_button:
        import_csv = st.button("📥 Import CSV")

# Se clicou no botão, abre o uploader
if import_csv:
    st.sidebar.subheader("⬆️ Upload de CSV")
    uploaded_file = st.sidebar.file_uploader("Escolha seu arquivo CSV", type="csv")

    if uploaded_file is not None:
        try:
            df_csv = pd.read_csv(uploaded_file)
            st.sidebar.success("✅ CSV carregado com sucesso!")
            st.sidebar.dataframe(df_csv)

            # Aqui você pode processar e transformar em formato igual ao do JSON/planilha
            st.subheader("📦 Dados importados do CSV")
            st.dataframe(df_csv)

            # ➕ (Opcional) Processar para incluir no fluxo de comparação, buscas etc.
